...
def extract_positive_samples(gt_num, gt_img, gt_data, gt_tp_savepath, param):
    ...
    for v in range(0,len(gt_data)):
        ...
        #if x >=0 and y >=0 and x_gt + r_gt +1 <= gt_img.shape[0] and y_gt + r_gt +1 <= gt_img.shape[1] and 2 * r_gt >= param.dmin and 2 * r_gt <= param.dmax : 
...
def extract_negative_samples(gt_num,gt_img, gt_mask, np, gt_fn_savepath, param, circleMask = False):
    ...
    while nn < np:
        ...
        if s_x >= 0 and s_y >=0 and e_x <= gt_img.shape[0] and e_y <= gt_img.shape[1]:
            ...
            if threshold <= param.thresh_overlay:
                ...
                if circleMask:
                    crop_circle_mask = np.zeros(crop_img.shape, dtype='uint8')
                    cv.circle(crop_circle_mask, (aux_x, aux_y), aux_r, (255, 255, 255), -1)
                    masked_crop = np.bitwise_and(crop_img, crop_circle_mask)
...
if __name__ == "__main__":
    ...
    for gt_num in gt_list:
        ...
        gt_slices_path = os.path.join("crater_data", "slices","org" , "tile" + gt_num)
        gt_mask_slices_path = os.path.join("crater_data", "slices","mask" , "tile" + gt_num)
        ...
        # extract positive and negative samples from threshold org images and save it on th_org folder on samples directory.
        num_tp_samples, gt_mask = extract_positive_samples(gt_num, gt_th_mask_img , gt_data, th_mask_tp_savepath) # original photo 
